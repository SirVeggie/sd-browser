...
- **Textarea autosize** (`SvGenField.svelte`): reactive remasure must read `field.value` (then `tick()` → `autosize`). Depending only on `useTextarea` leaves height stale after Use params / other store-driven value writes; `on:input` alone does not cover those paths. Remasure only when the **string** value changes — `discoverCards` rebuilds field objects on every workflow write (LoRA toggle, etc.); re-running `height: auto` on unchanged prompts jumps column scroll.
