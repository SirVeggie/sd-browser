...
- **Layouts:** no masonry — each column is its own scroll container. Card placement is stored **per column count** (auto 1 & 2; 3-col layout only after user drops a card into col 3; remove 3-col layout when col 3 is empty). Collapse / field order / hidden fields are shared across column counts. **Initial auto order** (`cardOrder.ts`): seeds → models → prompts → negative prompts → sampler → resolution → lora → images → others (alphabetical title tie-break). **Unsaved opens** (image / Comfy): if ≥50% of cards identity-match a saved workflow (type+title, not fields; `layoutInherit.ts`), remap that saved layout onto the new node ids (best match wins). Field order / hidden / int-control keys remap by widget-name fallbacks so adding widgets in Comfy does not break node matching.
