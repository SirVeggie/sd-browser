...
- **Output node previews** (`PreviewImage`, `SV-Danbooru*`, …): cards with `imageDisplay` show the last Comfy `executed` output via `/api/svgen/comfy/view` (same auth blob cache as pickers). `SvGenPanel` handles `onExecuted` → `svgenNodePreviewsStore` keyed by `sessionId\\0nodeId` (`nodePreviews.ts`); match both `node` and `display_node`; map `prompt_id` → session at queue time so mid-run session switches stay correct. Cleared when the session closes; mid-step latent binary WS frames are still dropped by the bridge. Also feeds SD Browser random-mode picker previews.
