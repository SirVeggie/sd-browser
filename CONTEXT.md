...
- Non-chrome `Select` panels are viewport-fixed (see Select note above) so column overflow cannot clip them.
- Collapsible `.wrapper .inner` keeps `overflow: hidden` while animating, then `overflow: visible` after open. Flyout extras use `.join-prev` so the lead rule isn’t doubled on top of the toggle row’s border.
...
- **Comfy ↔ panel bridge:** sd-browser → Comfy graph is `POST /sv_sd_browser/open_workflow` (Comfy polls `pending_workflow`). Reverse: Comfy SV Gen panel **sd-browser** button → `POST /sv_sd_browser/open_in_panel`; sd-browser polls `/api/svgen/pending-panel-workflow` → `pending_panel_workflow` (TTL ~60s, one-shot consume) and loads into the Generate panel (opens flyout + Generate tab).
...
- **Generate toolbar** lives with the Generate button (bottom): Stop (■), queue count `x`, Infinite, Keep one queued. Top bar keeps workflow load/save + **Queue** toggle (active count). Queue list shows running → pending → history with durations (WS `execution_*` timestamps); Infinite tops up to 1 (or 2 with Keep one queued); turning Infinite off while Keep one queued clears panel-owned pending prompts.
- **Toolbar / picker dropdowns:** workflow Load (`SvGenQueueBar`) and SD Browser sort (`SvGenSdBrowserImageModal`) use shared `Select` (viewport-fixed panel, same option chrome as settings/bulk) — not native `<select>`. Load stays an action menu (trigger label resets to `Load…` after pick).
- **Multi-open sessions (planned A1):** replace name field + Load with a session dropdown (recessed input chrome). Menu = open sessions (switch/close) → separator → saved workflows listed directly (click opens; × deletes from library without closing open copies). Opening a saved workflow creates an **unlinked copy** with that name (not tied to saved id); Save always uses a name dialog, with an overwrite warning when the name matches an existing saved workflow. If an open session already has that name, new open gets `Name (2)`, `Name (3)`, …. No dirty dots / Save* — seeds change constantly so dirty would almost always be true. Outside entry points (Open in panel / Comfy) also add open sessions. No Comfy status text in the bar — use `notify()` for offline / convert failures. Mockup: `tmp-mockups/multi-workflow-chrome.html`.
- **Persistence:** `LOCAL_DATA/svgen.sqlite3` (workflows + layouts), not MiscDB.
