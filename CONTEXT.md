...
- **Comfy errors are objects:** `/prompt` and convert return `{ error: { type, message, details }, node_errors }` (WS `execution_error` uses `exception_message`). Never `String(error)` — that is `[object Object]` in the panel. Format with `formatUnknownError` / `reportSvgenError` (`src/lib/svgen/formatError.ts`); `reportSvgenError` also `console.error`s the raw value. Proxy APIs attach the original payload as `detail`.
- **Panel error banner:** Generate still clears immediately. A successful queue (including infinite auto top-up) dismisses the current banner after `ERROR_DISMISS_GRACE_MS` from when the error appeared (`errorDismiss.ts`) so auto-queue does not hide it in the same moment. A newer error cancels the pending dismiss.
...
- **Convert + subgraphs:** `workflow_converter.py` applies outer promoted values onto expanded inners **positionally** (`proxyWidgets` when present, else the same inputNode→interior reconstruction as field discovery), then the same positional zip for API mapping (including non-proxied inners like `SV-LLMArgs`). Coerce bool→0/1 for FLOAT/INT when the saved array has a bool in a numeric slot (LLMArgs `repetition_penalty: false` → `0.0` in Save API).
- **INT control-after-generate:** Only slots that had a `fixed|randomize|…` companion in `widgets_values` show freeze + mode controls (`SvGenIntControl`) — not every INT (Primitive “Integer” has no companion; “Int” does). Modes live in layout `intControlModes` (default `randomize`); freeze/last-used live in the open-session bag (`svgenFrozenSeedsStore` / `svgenLastUsedSeedsStore`, restored with localStorage tabs). After each queued prompt, advance values via `applyIntControlsAfterQueue` and re-convert for the next queue item — do not submit the same seed N times when mode is randomize/inc/dec. In `queueOnePrompt`, convert/submit uses a pre-await workflow snapshot. Advance INT controls on the **live** `svgenSessionStore` workflow inside `update()` **in the same turn as that snapshot, before any await** — a second Generate click otherwise queues the same seeds. Writing the snapshot back after I/O would wipe edits typed during convert/submit (infinite top-up caret jump / lost chars). Convert/submit failure still leaves seeds advanced (already claimed). Controlled `value={…}` fields only reset the DOM when the string actually changes, so preserving live values also keeps the caret.
...
- **Pending number edits at queue time:** `<input type="number">` fires `change` on blur, not while typing. Ctrl+Enter is a capture-phase listener and does not blur (infinite top-up neither). Do **not** dispatch `change` / write the session store at queue time — that refreshes the field and steals focus. Peek live DOM values onto the convert/submit **snapshot** only (`applyPendingFieldWrites` in `pendingFieldFlush.ts`; LoRA via `overlayLoraRowStrengths`). Strength inputs keep a focused draft string so session/int-control invalidation cannot reset `value={number}` mid-edit. Commit still happens on blur.
- **Textarea autosize** (`SvGenField.svelte`, `textareaAutosize.ts`): remasure when the **string** value changes (Use params / hydrate / typing) — `discoverCards` rebuilds field objects on every workflow write (LoRA toggle, etc.); re-running `height: auto` on unchanged prompts jumps column scroll. Skip while `clientWidth` is 0; remasure on wrap-width change (`ResizeObserver`) and `document.fonts.ready`. Refresh with the flyout already open used to measure once against an unsettled layout (or fallback font) and never retry. Do not remasure on height-only resize (loops with `height: auto`).
- **Column scroll must not jump on chrome clicks:** `SvGenComboPicker` must not use `scrollIntoView` for the active option (it scrolls `.column` ancestors) — adjust `.list` `scrollTop` only (`comboListScroll.ts`); search/trigger `focus({ preventScroll: true })`. Center the selected option in `.list` after `reposition` + `tick` (menu has no max-height until then); ↑/↓ keep nearest-edge. `SvGenEnablePill` uses `pointerdown` `preventDefault` so click does not focus-scroll the column (still keyboard-focusable via Tab), and `pinColumnScroll` (`pinColumnScroll.ts`) restores `.column` `scrollTop` after toggle — needed on mobile when a text field is focused (blur / soft-keyboard / rediscovery still nudges nested overflow). On touch/pen, blur any focused text control *before* that `preventDefault` — otherwise focus stays in the field and the soft keyboard reopens on every toggle. Columns also set `overflow-anchor: none`.
...
- **Lora Tag Loader** (`SV-LoraTagLoader`): card-level special UI (`card.loraTagLoader`, field kind `lora_tags`) — not a plain textarea. Detect from node class / inner schema type in the saved UI workflow; **do not** require Comfy `object_info` or convert (those are queue-time). V3 exports often omit `input.widget` on STRING — still treat the text slot as `lora_tags` when Comfy is down. Filename combo options still come from `LoraLoader` `object_info` when available. `SvGenLoraTagLoader` rows: enable pill + LoRA combo (`LoraLoader`/`LoraLoaderModelOnly` `lora_name` options) + strength + optional clip strength. Header pill is a **master override** (`loraTagMasterEnabled[nodeId]`, default on) — off disables every tag at queue/convert only (`applyLoraTagMasterOverrides`); it does **not** rewrite per-row enables in the text. Per-row disables stay as `<#lora:name:…>` so Comfy’s `<l\w+:…>` matcher skips them while strengths are kept. Edit mode is custom: add row, × delete, drag reorder (rewrites tag order in `text`); **no Hide**. Add LoRA inserts a **disabled** row with no name (`<#lora::1>`) so the combo opens at the top-level folder — do not preselect `options[0]`. Clip-strength column + edit-mode clip toggle only when the CLIP socket is wired (`clipInputWired`); layout pref `loraClipStrength[nodeId]` (default on; `false` = omit `:clip` and use model strength). Parse/serialize: `loraTagText.ts`. **Triggers** (edit bar, between clip toggle and Add LoRA): opens a modal of copyable square chips (not gallery `Tag` pills). Omit LoRAs with no triggers. Empty result shows “No loras with trigger words found”; Close stays at the bottom of the modal min-height (`SvGenLoraTriggersModal`). Two or more words get a leading **copy all** chip (comma-separated). Comfy `POST /sv_sd_browser/lora_triggers` (proxied `/api/svgen/lora-triggers`) resolves names like the node (exact path/stem, then prefix) and returns `{ [requestedName]: string[] }`. Sidecar `{stem}.txt` next to the file wins; else `triggers.txt` in that folder then up to 2 parent hops. Catalog headings match the resolved stem with the same exact/prefix rules. The panel looks up the map with those rules (`loraTriggers.ts`) so partial row names still hit.
- **Output node previews** (`PreviewImage`, `SV-Danbooru*`, …): cards with `imageDisplay` show the last Comfy `executed` output via `/api/svgen/comfy/view` (same auth blob cache as pickers). Subgraph shells that promote `$$canvas-image-preview` also set `imageDisplay` when they have no image picker field; preview lookup accepts converter ids `outerId:innerId`. `SV-PreviewText` / `SV-WorkflowTimer` are `textDisplay` — same keep-even-with-no-fields rule; text comes from `executed` `output.text` (tuple/array) into `svgenNodeTextPreviewsStore`. `SvGenPanel` handles `onExecuted` → preview stores keyed by `sessionId\\0nodeId` (`nodePreviews.ts`); match both `node` and `display_node`; map `prompt_id` → session at queue time so mid-run session switches stay correct. Cleared when the session closes; mid-step latent binary WS frames are still dropped by the bridge. Also feeds SD Browser random-mode picker previews.
- **Card reorder:** pointer drag via same geometry as tags/filters (`sortableGeometry` + `DragHandle` ghost/FLIP) in `SvGenColumns` — not HTML5 DnD. While dragging, holding near the top/bottom of a column auto-scrolls that column (`AUTO_SCROLL_*` in `SvGenColumns`) so long lists stay reachable. Cards live in a **per-column** keyed `{#each}`: moving into an earlier column creates the new slot first, then destroys the old one. `itemEls` must use `bindKeyedElement` (`keyedElementMap.ts`) so destroy does not `delete(id)` when the map already points at the new node — otherwise that card’s handle silently no-ops until another handle click re-renders. Pointer capture belongs on the columns root, not the handle (the handle remounts mid-drag).
...
- **Generate toolbar** lives with the Generate button (bottom): Stop (■), queue count `x`, Infinite, Keep one queued. Queue `x` uses the same horizontal `numberDrag` scrub as card number fields (`SvGenGenerateBar`, min 1 / max 99 / step 1). Top bar is session dropdown + Save / Queue / burger (Open in Comfy, JSON). Queue list shows running → pending → history with durations (WS `execution_*` timestamps); list scrolls after **4** visible rows (`SvGenQueueList`). Infinite tops up to 1 (or 2 with Keep one queued); turning Infinite off while Keep one queued clears panel-owned pending prompts.
- **Toolbar / picker dropdowns:** SD Browser sort (`SvGenSdBrowserImageModal`) uses shared `Select` (viewport-fixed panel portaled to `document.body`). Portaled `.panel.viewport` is `z-index: 10060` so it sits above picker overlays (`10050`). Do not style the panel with a descendant `:global(.select .panel)` under the modal — that selector dies after portal and the menu opens behind the overlay. Modal styles must target `:global(.select .trigger)` (not bare `.trigger`) so they beat Select’s scoped `border: none` / `padding: 0` / `font-size: 1em` — otherwise Sort renders oversized and unbordered next to Upload/Close. Session switcher is custom recessed dropdown in `SvGenQueueBar` (open sessions + saved list).
...
- **Autocomplete list:** hide the popup when every remaining match would leave the field unchanged (exact complete item as the only option, e.g. `contrapposto` after `1girl, contrapposto`). Rank: match group, then shorter matching segment, then higher `score` (booru post count), then shorter full value. Booru/a1111 parse converts the inserted value to generation form (`_` → space, `()` → `\(\)`); the original tag stays as an alias so underscore queries still match. Custom CSV/JSON values are left as written. Index stores source-file `mtime` and refreshes on search/settings list when the file still exists and mtime changed; missing file keeps the sqlite copy. Auto-refresh does not wipe items if the new file fails to parse. Manual Reindex still rebuilds in place (needed after parser changes when the file mtime did not change). `idleFade` missing in localStorage means on; default fade delay is 2000ms (settings UI is seconds with one decimal). Comma (and word) boundaries also break on newlines. Do not strip attention weights or trailing punctuation from the query (`(chromatic:1.5)` searches `chromatic:1.5)`, which must not match `chromatic`). Each walked query ignores leading non-word characters (anything other than letters, numbers, `_`, `-`) so `green (dress` still matches `dress` after the full phrase fails. Search while IME/composition is in progress — do not close on `compositionstart` or wait for space. Query caret is `max(selectionStart, selectionEnd)` so a composing range still sees the typed word. Inserted text opens/refreshes; caret movement closes. Delete refreshes if already open and does not open if closed (Ctrl+Space still opens). Touch select follows `solid`, not tap count: faded tap/drag only reveals; a tap while solid accepts; fading clears that. List/info scroll and an active touch drag keep `solid` and reset the fade timer (including momentum scroll).
