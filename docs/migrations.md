...
4. CPU parsing runs on a worker-thread pool (up to `min(cpus-1, 8)`). Main-thread work is pipelined one slice per event-loop turn (blobs-only load + clone, staging writes of 200) so HTTP/SSE stay responsive.
